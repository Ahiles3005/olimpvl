...
Это делаем вжесткую пока, хз, где берется

**/

$arParams["PRICE_CODE"] = array(  "vladivostok", "ussurijsk", "arsenev", "naxodka", "komsomolsk_na_amure" ) ;
...
$PRODUCT_KEY = 0;
$arProduct["NUMBER_OF_STORES"] = Olimp\Catalog::getCountStoreWithProduct($arOfferID, $arParams["PRICE_CODE"][0]);
// var_dump($arResult["OFFERS"]);
// 
//  die();
// echo "*********************";
// var_dump($arProduct["NUMBER_OF_STORES"]);
...
foreach ($arResult["PROPERTIES"]["PICTURES"]["VALUE"] as $key => $pathPic) {
    $arPicSmall = CFile::ResizeImageGet($pathPic, array('width' => 100, 'height' => 100), BX_RESIZE_IMAGE_PROPORTIONAL, false);
    $arProduct["PICTURES"]["PIC_SMALL"][$key] = $arPicSmall["src"];

    $arPicBig = CFile::ResizeImageGet($pathPic, array('width' => 680, 'height' => 398), BX_RESIZE_IMAGE_PROPORTIONAL, false);
...
while ($response = $resResponse->GetNext()) {
    ...
    if (!empty($response["PROPERTY_PARENT_COMMENT_VALUE"])) {
        ...
        $response["CREATED_USER_NAME"] = (!empty($city)) ? $arUser["NAME"] . " (" . $city . ")" : $arUser["NAME"];
