...
include 'parts/badges.php';

$a = !empty($_SESSION["CATALOG_COMPARE_LIST"][1]) ? count($_SESSION["CATALOG_COMPARE_LIST"][1]['ITEMS']) : 0;

function getLabels($section_id = 0, $ids = array())
{
	if (!empty($ids)) {
		$arrFilter = array(
			array(
				"LOGIC" => "OR",
				array("ID" => $ids),
				array("PROPERTY_RAZDELZ_VALUE" => $section_id)
			),
			"IBLOCK_ID" => 17,
			"ACTIVE" => "Y"
		);
		...
		$arrFilter = array(
			"PROPERTY_RAZDELZ_VALUE" => $section_id,
			"IBLOCK_ID" => 17,
			"ACTIVE" => "Y"
		);
	}
	$arrSelect = array("PREVIEW_PICTURE", "ID", "NAME", "PREVIEW_TEXT", "PROPERTY_LINK_OUT", "PROPERTY_RAZDELZ", "PROPERTY_RAZDELZ.ID");

	$out = array();
	$elem = CIBlockElement::GetList(array(), $arrFilter, false, false, $arrSelect);
	while ($el = $elem->GetNextElement()) {
		$el = $el->GetFields();

		if (in_array($section_id, $el['PROPERTY_RAZDELZ_VALUE']) || in_array($el['ID'], $ids)) {
...
$labels = getLabels($arResult['IBLOCK_SECTION_ID'], $arResult['PROPERTIES']['TOVAR_LABEL']['VALUE']);
	...
	<div id="tab-carousel-product"
		...
		<div
			...
			<? if (count($arResult["PRODUCT"]["PICTURES"]["PIC_SMALL"]) == 0) : ?>
				<a2 class="item<? if ($key == $arResult["PRODUCT"]["PICTURES"]["MAIN"]) echo " active" ?>" data-fancybox-group="gallery" href="<?= DEF_PIC_DETAIL ?>">
					<img src="<?= DEF_PIC_DETAIL ?>" alt="..." title="">
				</a2>
			<? else: ?>
				<? $originalPics = $arResult["PRODUCT"]["PICTURES"]["ORIGINAL"];
				foreach ($arResult["PRODUCT"]["PICTURES"]["PIC_BIG"] as $key => $src): ?>
					<a2 class="item<? if ($key == $arResult["PRODUCT"]["PICTURES"]["MAIN"]) echo " active" ?>"
							data-fancybox-group="gallery" href="<?= $originalPics[$key] ?>">
						<img src="<?= $src ?>" alt="..." title="">
					</a2>
				<? endforeach ?>
			<? endif ?>
				...
				<div class="b-tab__accordion--item">
					...
					<div class="b-tab__accordion--text collapse">
						<div class="b-tab-description">
							...
							<? if (!empty($arResult["PROPERTIES"]["SIZE_TABLE"]["VALUE"])): ?>
								<h3><?= GetMessage("SIZE_TABLE") ?></h3>
								<a href="<?= CFile::GetPath($arResult["PROPERTIES"]["SIZE_TABLE"]["VALUE"]) ?>"
									 target="_blank" title="<?= GetMessage("SIZE_TABLE") ?>"
									 class="b-tab-description__link-download"><?= GetMessage("GET_SIZE_TABLE") ?></a>
							<? endif ?>
					...
					<div class="b-tab__accordion--text collapse">
						<div class="b-tab-review clearfix">
							<div class="b-tab-review__add-review">
												...
												<div class="modal-body b-popup__modal-body">
													<div class="b-popup-feedback__form">
														<form class="b-form" type="POST" action="">
															...
															<? } else { ?>
																<input type="hidden" id="text-name" name="UNAME" value=''>
								...
								<? else: ?>
									<a href="#popup-login" class="b-tab-review__add-rating"
										 data-popup="open">Добавить
																			 отзыв</a>
...
<div class="b-product-info">
	<div class="b-product-info__head">
		...
		<h1 class="b-product-info__name"><?= $arResult["NAME"] ?></h1>

		<div class="b-product-info__article">
			<?= GetMessage("ARTICUL") . " " . $arResult["PROPERTIES"]["ARTICUL"]["VALUE"] ?>
		</div>
		<div class="b-product-info__id">
			<?= GetMessage("ID")?> <span class="copy-click" data-mess="Код скопирован"><?= $arResult["ID"] ?></span>
		</div>
	...
	<div class="b-product-info__form">
		<form action="#" method="post">
			...
			<div class="b-product-info__action">
				...
				<? if (!empty($labels)) {
					...
					foreach ($labels as $label) {
						...
						if (!empty($label['PROPERTY_LINK_OUT_VALUE'])) {
							?>
							<a id="mgc-label-id<?= $label['PROPERTY_LINK_OUT_VALUE'] ?>" class="mgc-label"
								 href="<?= $label['PROPERTY_LINK_OUT_VALUE'] ?>" target="_blank"><img
										src='<?= $label["PREVIEW_PICTURE"] ?>'></a>
							<?
						} else {
							?>
							<a type="button" data-id="<?= $label['ID'] ?>" href="#popup-label-info" data-popup="open"
								 id="mgc-label-id<?= $label['ID'] ?>" class="mgc-label mgc-label-click" href="#"><img
										src='<?= $label["PREVIEW_PICTURE"] ?>'></a>
							<span style="display:none" id="mgc-label-name-id<?= $label['ID'] ?>"><?= $label['NAME'] ?></span>
							<span style="display:none" id="mgc-label-info-id<?= $label['ID'] ?>"><?= $label['PREVIEW_TEXT'] ?></span>
			...
			<div class="b-product-info__price">
				...
				<?
				$GLOBALS['fb']->fb($arResult["PRODUCT"]["NUMBER_OF_STORES"], "NS");
				?>
				<div
					...
					<? if ($arPrice = reset($arResult["OFFERS"][$arResult["PRODUCT"]["KEY"]]["PRICES"])): ?>
                        ...
                        <span class="b-rouble">q</span></span>
							<span class="b-product__priceold-line-through b-product__priceold-line-through--product"><span
										class="b-product__priceold"><?= round($arPrice["VALUE"]) ?><span
						...
						<? else: ?>
							<span
									class="b-product__pricenew b-price-large b-price-large--product"><?= round($arPrice["VALUE"]) ?><span
				...
				<div class="do-login-register">
					<a class="btn b-btn__default" href="#popup-low_price" data-popup="open" style="color:#777">Сообщить о снижении
																																																		 цены</a> &nbsp; <a
							class="btn b-btn__default" href="#popup-found_cheaper" data-popup="open" style="color:#777">Нашли
																																																					дешевле?</a>
				</div>
				...
				<? if ($arResult["PRODUCT"]["NUMBER_OF_STORES"][$arResult["PRODUCT"]["ID"]]):
					...
					if (false) { ?>
						...
						<a type="button" href="#popup-login" data-popup="open"
							 class="btn b-btn__default b-btn__default--sm b-btn__default--product mgc-btn-login-before-cart"
						/><?= GetMessage("IN_CART") ?></a>
					...
					<? } else { ?>
						...
						<button name="onebuy" data-id="<?= $arResult["PRODUCT"]["ID"] ?>" onclick="oneBuy(this)"
										class="btn b-btn__default b-btn__default--sm b-btn__default--product b-btn__onebuy">
							<span><?= GetMessage("ONE_BUY") ?></span>
						</button>
			...
			<div class="add-to-wish-n-compare">
				<button type="button" data-id="<?= $arResult["PRODUCT"]["ID"] ?>"
								class="mgc-add-compare"><?= (!empty($_SESSION["CATALOG_COMPARE_LIST"][1]['ITEMS'][$arResult["PRODUCT"]["ID"]])) ? 'В сравнении' : 'Добавить в сравнение'; ?></button>
				<button type="button" data-id="<?= $arResult["PRODUCT"]["ID"] ?>" class="bx_big bx_bt_button h2o_add_favor">
...
<div class="b-goods">
	...
	ML_TYPE' => array(''),
	'ELE<? if (!empty($arResult["PROPERTIES"]["RECOMMENDED"]["SECTIONS_ID"])): ?>
		<?
...
$GLOBALS['DELTA'] = $GLOBALS['SCRIPT_END'] - $GLOBALS['SCRIPT_START'];

echo "DELTA";
var_dump($GLOBALS['DELTA']);
