			...
			authFormNode = BX.create('DIV', {
				...
				children: [
					...
					BX.create('A', {
						...
						href:'/personal-area/recovery.php?clear_cache=Y'
						...
						BX.create('A', {
							...
							events: {
								click: BX.delegate(function(e){
                                                                  ...
                                                                  //  console.log($("#popup-sign-up").html());
                                                                  
                                                                    $(".siteusers").find("a").trigger("click");
                                                                //    $.fancybox.open($("#popup-sign-up").html());
    ...
    opts : {
       ...
       */                                                     
		...
		editBasketItemsHeader: function(basketItemsNode)
		{
			...
			console.log(this.result.GRID.HEADERS);
		...
		editPaySystemInfo: function(paySystemNode)
		{
			...
			if (currentPaySystem.ID == 3 && !this.params.AVALIABLE_ONLINE_PAYMENT) {
				avaliable = false;
				BX.addClass(
					paySystemInfoContainer,
					'error'
				);
				...
				errorBlock = BX.create('DIV',
					{
						props: {
							className: 'bx-soa-pp-company-error'
						},
						html: 'Нужно сделать запрос в магазин, после этого вы сможете оплатить заказ в личном кабинете'
					}
				);
			}
			...
			if (currentPaySystem)
			{
				...
				title = BX.create('DIV', {
					...
					children: [BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: currentPaySystem.DESCRIPTION}), errorBlock]
				});
		...
		insertDateProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				for (i = 0; i < inputText.length; i++){
					inputText[i].onkeydown = function(e){
						e.preventDefault()
					};
            ...
            if (delivery.ID == 4) {
                 var deliveryNode = BX.findParent(propContainer, {className: 'form-group'});
                 var courierText = BX.create('DIV', {
                     props: {className: 'courier-text'},
                     html: this.params.COURIER_TEXT
                 });
                 deliveryNode.appendChild(courierText);
             }
		},
		...
		alterDateProperty: function(settings, inputText)
		{
			...
			BX.bind( parentNode.querySelector('input[type=text]'), 'click', BX.delegate(function(e){
				var target = e.target || e.srcElement,
			...
			BX.bind( parentNode.querySelector('input[type=text]'), 'bxchange', BX.delegate(function(e){
				var target = e.target || e.srcElement,
					parentNode = BX.findParent(target, {tagName: 'DIV', className: 'input-group'});
		...
		getDaysForDelivery: function()
		{
			var delivery = this.getSelectedDelivery();
			var payment = this.getSelectedPaySystem();
			var totalPrice = parseInt(this.result.TOTAL.PRICE_WITHOUT_DISCOUNT_VALUE);
			var currentDay = false;
			var days = 5;

			if (delivery.ID == 3) {  // самовывоз
				if (payment.ID == 1 || payment.ID == 4) { // наличные
					days = 2;
					currentDay =  true;
					if (totalPrice > 20000) {
						days = 1;
					}
				} else if (payment.ID == 3) { // онлайн
					days = 10;
				}
			} else if (delivery.ID == 4) {  // курьер
				days = 5;
			}
		...
		{
			var daysCount = this.getDaysForDelivery();

			$(document).find(".bx-calendar-cell").removeClass("bx-calendar-date-hidden disabled");
			var el = $('[id ^= "calendar_popup_"]'); //найдем div  с календарем
			var links = el.find(".bx-calendar-cell"); //найдем элементы отображающие дни


			$('.bx-calendar-left-arrow').attr({'onclick': 'BX.Sale.OrderAjaxComponent.changeCalendar();',}); //вешаем функцию изменения  календаря на кнопку смещения календаря на месяц назад
			$('.bx-calendar-right-arrow').attr({'onclick': 'BX.Sale.OrderAjaxComponent.changeCalendar();',}); //вешаем функцию изменения  календаря на кнопку смещения календаря на месяц вперед
			$('.bx-calendar-top-month').attr({'onclick': 'BX.Sale.OrderAjaxComponent.changeMonth();',}); //вешаем функцию изменения  календаря на кнопку выбора месяца
			$('.bx-calendar-top-year').attr({'onclick': 'BX.Sale.OrderAjaxComponent.changeYear();',}); //вешаем функцию изменения  календаря на кнопку выбора года

console.log(this.params,"DATE_UNIX");

	var oneDay = 24*60*60*1000;
    
			var d = parseInt(this.params.DATE_UNIX)*1000 - oneDay;

		
			var dateMax = d+parseInt(oneDay*daysCount.days);
			var dateMin = d;
			...
			for (var i =0; i < links.length; i++)
			{
				...
				if (atrDate >= dateMax || atrDate <= dateMin) {
					$('[data-date="' + atrDate +'"]').addClass("bx-calendar-date-hidden disabled").on('click',function(){return false;}).on('dblclick',function(){return false;}); //меняем класс у элемента отображающего день, который меньше по дате чем текущий день
				}
			}
		},
		...
		editTotalBlock: function()
		{
			...
			{
				if (this.getSelectedDelivery().ID == 3) {
					priceHtml = '0 руб.';
				} else {
