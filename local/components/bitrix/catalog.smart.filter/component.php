...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		...
		else
		{
			...
			//измененная часть по задачи 24919 пункт 14, правки по фильтру наличия товаров
			$keyCity = (empty($_COOKIE['KEY_CITY'])) ? Olimp\City::getDefaultCityCode() : $_COOKIE['KEY_CITY'];
			foreach($arResult["ITEMS"] as $PID => $arItem){
				if ($arItem['CODE'] == 'AMOUNT_'.$keyCity){
					$keys = array_keys($arItem['VALUES']);
					$property_enums = CIBlockPropertyEnum::GetList(Array(), Array("IBLOCK_ID" => IBLOCK_ID_CATALOG, "CODE" => $arItem['CODE']));
					while($enum_fields = $property_enums->GetNext()){
						if(!in_array($enum_fields["ID"], $keys)){
							$key = $this->fillItemValues($arResult["ITEMS"][$arItem["ID"]], $enum_fields["ID"]);
						}
					}
				}
			}
