...
if($this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
	...
	if(is_array($arParams["UF_FILTER"]) && count($arParams["UF_FILTER"]))
	{
	    foreach($arParams["UF_FILTER"] as $fCode=>$fVal)
	    {
	        $arFilter[$fCode] = $fVal;
	    }
	}
	...
	//ORDER BY
	$arSort = (empty($arParams["ORDER"])) ? array("left_margin"=>"asc") : $arParams["ORDER"];
	...
	if (isset($arParams["BRAND_CODE"]) && !empty($arParams["BRAND_CODE"])) {
		...
		$rsElem = CIBlockElement::GetList(array(), $arFilter);
		if ($arElem = $rsElem->GetNextElement()) {
 			$arBrand = $arElem->GetFields();
		}
		...
		$arBrandFilter = array(
			...
			'GLOBAL_ACTIVE' => 'Y',
			'PROPERTY' 		=> array(
				$arParams["FILTER_PROPERTY"] => $arBrand["ID"]
			)
		);
		$rsSections = CIBlockSection::GetList(array('LEFT_MARGIN' => 'asc'), $arBrandFilter, true);
	} else {
	
	if(!empty($arParams["SECTION_ID"]) && intval($arParams["SECTION_ID"]))$arFilter['SECTION_ID'] = $arParams["SECTION_ID"];
	
		$rsSections = CIBlockSection::GetList($arSort, $arFilter, $arParams["COUNT_ELEMENTS"], $arSelect);
	}
	$rsSections->SetUrlTemplates("", $arParams["SECTION_URL"]);

	$sectionsIgnore = array();		// Массив идентификаторов для игнорирования разделов верхнего уровня
	while($arSection = $rsSections->GetNext())
	{
		/*
		if (isset($arParams["BRAND_CODE"]) && !empty($arParams["BRAND_CODE"])) {
			if (in_array($arSection["ID"], $sectionsIgnore))
				continue;
			else
				$sectionsIgnore[] = $arSection["IBLOCK_SECTION_ID"];
		}
		*/
...
if ($arParams['SEO'] == 'Y') {
	...
	//SET TITLE
	if ($arSection["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"] != "")
		$APPLICATION->SetTitle($arSection["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"], $arTitleOptions);
	elseif(isset($arParams["SECTION_NAME"]))
		$APPLICATION->SetTitle($arParams["SECTION_NAME"], $arTitleOptions);

	//BROWSER_TITLE
	$browserTitle = \Bitrix\Main\Type\Collection::firstNotEmpty(
		$arResult, $arParams["BROWSER_TITLE"]
		,$arSection["IPROPERTY_VALUES"], "SECTION_META_TITLE"
	);
	...
	elseif ($browserTitle != "")
		...
		$APPLICATION->SetPageProperty("title", $arParams["SECTION_NAME"], $arTitleOptions);

	//KEYWORDS
	$metaKeywords = \Bitrix\Main\Type\Collection::firstNotEmpty(
		$arResult, $arParams["META_KEYWORDS"]
		,$arSection["IPROPERTY_VALUES"], "SECTION_META_KEYWORDS"
	);
	...
	$metaDescription = \Bitrix\Main\Type\Collection::firstNotEmpty(
		...
		,$arSection["IPROPERTY_VALUES"], "SECTION_META_DESCRIPTION"
	);
