...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	//измененная часть по задачи 24919 пункт 5, правки по фильтру
	$tempArrFilter = [];
...
//  exit;	
 //	echo "-->";
	foreach ($arrFilter as $key => $filt) {
		if(($key == '=PROPERTY_84') || ($key == '=PROPERTY_86') || ($key == '=PROPERTY_87') || ($key == '=PROPERTY_88') || ($key == '=PROPERTY_142')) {
			$tempArrFilter[$key."_ENUM_ID"] = $filt;
		}elseif((strripos($key, "PROPERTY_78") !== false) || (strripos($key, "PROPERTY_80") !== false) || (strripos($key, "PROPERTY_81") !== false) || (strripos($key, "PROPERTY_82") !== false) || (strripos($key, "PROPERTY_141") !== false)) {
			
			if($filt==1)$filt = '';
			
			$tempArrFilter[$key."_VALUE"] = $filt;
		}else{
			$tempArrFilter[$key] = $filt;
		}
	}
	
	$arrFilter = $tempArrFilter;
	$keyCity = (empty($_COOKIE['KEY_CITY'])) ? Olimp\City::getDefaultCityCode() : $_COOKIE['KEY_CITY'];
	ini_set('max_execution_time', 90);
	if(!empty($arrFilter["OFFERS"]["=PROPERTY_".CATALOG_SECTION_FILTR_SIZE_PROPERTY."_VALUE"]) ){
	
	
	

		//выбираем все элементы для фильтрации по складам
		$rsElements = CIBlockElement::GetList($arSort, array_merge($arrFilter, $arFilter), false, $arNavParams, $arSelect);
		while($arItem = $rsElements->GetNext())
		{
			$idsAllProducts[$arItem['ID']] = (int)$arItem['ID'];
		}

		//выбираем торговые предложения для фильтрации по торговым предложениям
		$res = CCatalogSKU::getOffersList($idsAllProducts, 0, array(), array(), array("CODE"=>array('SIZE'))); 
...
// 					if(in_array($offer["PROPERTIES"]["SIZE"]["VALUE"],$arrFilter["OFFERS"]["=PROPERTY_".CATALOG_SECTION_FILTR_SIZE_PROPERTY])){
// 						$idFromSize[$key] = $key;
// 						break;
// 					}
// 				}
...
// 		}
		...
		$arProduct["NUMBER_OF_STORES"] = Olimp\Catalog::getCountStoreWithProduct($arOfferID, isCity( $keyCity ) );
		...
		//"IBLOCK_ID"=>"-1" в случае если нет подходящих товаров, чтобы не затрагивать код ниже, неизмененной части
		if(!empty($idFromSize))
			$arFilterNew = Array("IBLOCK_ID"=>IBLOCK_ID_CATALOG, "ID" => $idFromSize);
		else
			$arFilterNew = Array("IBLOCK_ID"=>"-1");
	}else{
	
	
		//$arFilterNew = array_merge($arrFilter, $arFilter);
		//выбираем все элементы для фильтрации по складам
		...
		//выбираем торговые предложения для фильтрации по торговым предложениям
		$res = CCatalogSKU::getOffersList($idsAllProducts, 0, array(), array(), array()); 
...
foreach ($res as $key => $item) {
	...
	$arFilterNew["CHECK_PERMISSIONS"] = "N";
	
//  	var_dump($idsAllProducts);
//  	exit;



	$rsElements = CIBlockElement::GetList($arSort, $arFilterNew, false, $arNavParams, $arSelect);
...
// 	exit;
	...
	)
	{
		...
		$arProductNumberOfStores = Olimp\Catalog::getCountStoreWithProduct($ids, isCity($keyCity));
